...
def findformdata(dlist, key, exact=True, limit=None):
    ...
    """
    # If there is no processing wrapper function, just create a function that
    # returns its input (so that we can just assuming we're always calling the fn)
    if not limit: limit = lambda v: True
    
    findings = defaultdict(list)
    
    for domain in dlist:
        # Correct type                               # interested in this       # has a form
        if issubclass(type(domain), userdata.Service) and limit(domain) and hasattr(domain, 'formdata'):
            if exact: 
                # Match exact formdata key
