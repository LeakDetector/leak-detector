...
class LeakResults(object):
    ...
    def analyze(self, again=False):
        ...
        if not self.analysis_finished and not again:
            # List of function names generated by decorators --> list of actual functions bound to this
            # class instance.
            ordered_func_list = [getattr(self, fn) for fn in # grab actual function
                                    reduce(list.__add__, # reduce to 1d list
                                        [fname for order, fname in # (n, fname) --> ordered list
                                            sorted(self._analyses.items(), key=lambda item: item[0]) 
                                        ]
                                    )
                                ] 
        
            [function() for function in ordered_func_list] # call all functions
